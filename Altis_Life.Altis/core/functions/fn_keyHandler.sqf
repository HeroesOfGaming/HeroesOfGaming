...
switch (_code) do {
	...
	case 24: {    
		...
		if (soundVolume != 1) then {
			1 fadeSound 1; titleText ["Your sound has returned to normal.", "PLAIN"]; 
		} else {
			1 fadeSound 0.4; titleText ["Your sound has been lowered.", "PLAIN"];
			};
		};
	};
	...
	case _interactionKey: {
		if((!life_action_inUse) && (vehicle player == player)) then {
			if(life_inv_pickaxe > 0) then {[] spawn life_fnc_pickAxeUse};
		};
	...
	case 15:{	if (!_alt && !_ctrlKey && playerSide == west) then	{		[] call life_fnc_radar;	};}; 
